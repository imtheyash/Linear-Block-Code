...
for i in range(row):
p = list(map(int, input(f"Enter the row values {i+1} (Separated by space):
").split()))
if len(p) != col:
print("Error: Number of columns doesn't match the entered parity bit
count.")
exit()
pb.append(p)
...
m = np.array([[1 if (i >> (k - j - 1)) & 1 else 0 for j in range(k)] for i in
range(2 ** k)])
...
rc = list(map(int, input(f"\nEnter the received codeword (space separated {n}
bits): ").split()))
if len(rc) != n:
...
formatted_row = " ".join(map(str, combined_row[:hp1.shape[1]])) + '\t' + "
".join(map(str, combined_row[hp1.shape[1]:]))
print(formatted_row)
...
```
# OUTPUT:
![lcb 1](https://github.com/user-attachments/assets/0f199084-ba4f-4e4e-a7bc-63d10a0de22b)
![lcb 2](https://github.com/user-attachments/assets/94ee9656-b7e7-484b-abd9-a61494b88291)


# RESULT:
Thus ,The linear block code was successfully implemented for error detection and singlebit error correction.
